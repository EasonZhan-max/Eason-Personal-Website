...
const normalizePayload = (value: unknown): Omit<StatusRecord, 'received_at_ms' | 'signature_timestamp_ms'> => {
  ...
  const reportedLastOnline = online ? '' : cleanDate(value.last_online_at);
  const reportedLastOnlineMs = reportedLastOnline ? Date.parse(reportedLastOnline) : 0;
  return {
    online,
    ...
    last_online_at_ms: Number.isFinite(reportedLastOnlineMs) ? reportedLastOnlineMs : 0
  };
};
...
const normalizeStored = (value: unknown): StatusRecord | null => {
  ...
  try {
    ...
    const storedLastOnlineAt = typeof value.last_online_at_ms === 'number' ? value.last_online_at_ms : Number.NaN;
    const lastOnlineAt = Number.isFinite(storedLastOnlineAt)
      ? storedLastOnlineAt
      : normalized.online ? receivedAt : 0;
    return {
      ...normalized,
      last_online_at_ms: lastOnlineAt,
...
const getPublicStatus = async (env: Env): Promise<Response> => {
  ...
  if (!stored) {
    return jsonResponse({
      success: true,
      data: { configured: false, available: true, online: false, updated_at: '', last_online_at: '', profile: null }
    });
  }
  ...
  return jsonResponse({
    ...
    data: {
      ...
      last_online_at: stored.last_online_at_ms > 0 ? new Date(stored.last_online_at_ms).toISOString() : '',
      profile
    }
  });
};
...
const updateStatus = async (request: Request, env: Env): Promise<Response> => {
  ...
  const record: StatusRecord = {
    ...normalized,
    last_online_at_ms: normalized.online
      ? receivedAt
      : previous?.last_online_at_ms || normalized.last_online_at_ms || (previous?.online ? previous.received_at_ms : 0),
    received_at_ms: receivedAt,
    signature_timestamp_ms: signatureTimestamp
  };
